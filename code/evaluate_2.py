...
# from models import SSModelForCLM, LinearWithLoRA
...
class MetricUtils:
    ...
    @staticmethod
    def evaluate_bleu(dataloader: DataLoader, model: SSModelForCLMWithLoRA, device: torch.device, output_dir: Path) -> dict:
        model.eval()
        bleu_scores = []
        gen_refs = []  # For saving predictions and references
        smooth_fn = SmoothingFunction().method1
        
        tokenizer = model.tokenizer

        with torch.no_grad():
            with tqdm.tqdm(dataloader, desc="Evaluating BLEU...", colour="blue") as pbar:
                for i, batch in enumerate(pbar):
                    ...
                    #     label_mask = (labels[i] != -100).nonzero(as_tuple=True)[0]
                    #     if len(label_mask) == 0:
                    #         prompt_ids = input_ids[i]
                    #     else:
                    #         cutoff = label_mask[0].item()
                    #         prompt_ids = input_ids[i][:cutoff]
                    ...
                    for prompt in prompts:
                        gen_output = model.generate(prompt=prompt, gen_config=gen_config)
                        # print(gen_output)
                        outputs.append(gen_output["output_text"])
                        # print("Generated output:", gen_output["output_text"])
                    output_texts = outputs  # Assuming batch size is 1, otherwise handle accordingly
                    # print(f"Generated text: {output_texts[0]}")
                    generated_text=output_texts[0]  # Assuming batch size is 1
                    # generated_text = tokenizer.decode(generated_ids[0][input_length:], skip_special_tokens=True)
                    reference_text = tokenizer.decode(labels[0][labels[0] != -100], skip_special_tokens=True)
                    ...
                    print(f"BLEU score for example {i}: {bleu:.4f}")
                    bleu_scores.append(bleu)

        avg_bleu = sum(bleu_scores) / len(bleu_scores) if bleu_scores else 0.0
        results_summary = {
            "average_bleu": round(avg_bleu, 4),
            "generated_references": gen_refs,
        }

        MetricUtils._save_results(output_dir, results_summary, "bleu_summary")
        return results_summary
    @staticmethod
...
class LogEvaluator:
    ...
    def evaluate(self, metric: str) -> None:
        if metric == "perplexity":
            ...
            return MetricUtils.evaluate_bleu(self.dataloader, self.model, self.device, self.output_dir)
        else:
...
def main(model_name: str, device: torch.device) -> None:
    config = {
        ...
        "checkpoint_path": "/mnt/home-ldap/das_ldap/mambaProRCA/code/outputs/ckpt/mamba2-2.7b-hf_proactiveRCA/1.000_1.0e-05_r64", "checkpoint_name": "checkpoint-4580",
...
if __name__ == "__main__":
    ...
    main("AntonV/mamba2-2.7b-hf", device=device) 